    ...
    int gao() {
        ...
        for(int i = 0; i < num; i++) {
            ...
            for(int j = 0, p1 = 0, p2 = 0; j < n; j++) {
                ...
                while(p1 <= 2 * n && p[p1] < ple) p1++;
                while(p2 <= 2 * n && p[p2] < plele) p2++;

                //printf("%d: %d %d\n", j, p1, p2);
                
                ans = (ans + ((pt % mod) ^ (pre[p1] - pre[j])) % mod) % mod;
                ans = (ans - 2 * ((pt % mod) ^ (pre[p2] - pre[p1])) % mod) % mod;
            }
