...
int main() {
    ...
    for(int cas = 1; cas <= Te; ++cas) {
        ...
        for(int j = 0; j <= 3; ++j) {
            for(int i = 1; i <= n; ++i) {
                lsh[j][i*2-1] = seg[j][i].s.x;
                //lsh[j][i*4-2] = seg[j][i].s.x + 1;
                lsh[j][i*2] = seg[j][i].e.x;
                //lsh[j][i*4] = seg[j][i].e.x + 1;
            }
            lsh[j][2*n+1] = -M; lsh[j][2*n+2] = M;
            sort(lsh[j] + 1, lsh[j] + 2 * n + 3);
            siz[j] = unique(lsh[j] + 1, lsh[j] + 2 * n + 3) - lsh[j] - 1;
        }
