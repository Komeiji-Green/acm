...
unordered_map<state, BigInt, _hash, cmp> hsh, vis;

BigInt dfs(int num, int v, int idx) {
    if(num == 0) return 1ll;
    if(hsh.count(st)) return hsh[st];
    BigInt cur = 0ll; uint lbd, tmp, rem[18];
    bool flag = false;
    for(int i = 1, sz; i <= n; ++i) if(st.w[i]) {
        lbd = (st.w[i] & (-st.w[i]));
        if(lbd != bit[v]) continue;
        if(i < idx) return 0ll;   // 不是按顺序的不行
        flag = true;
        tmp = st.w[i]; st.w[i] = 0;
        sz = deg[i];
